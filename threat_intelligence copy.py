...
class ThreatIntelligence:
    ...
    def _calculate_threat_score(self, vt_data: Dict, abuse_data: Dict) -> Dict:
        """Calcula a pontuação de ameaça com base nos dados das APIs."""
        threat_score = 0
        details = []

        if "error" not in vt_data and vt_data.get("malicious", 0) > 0:
            threat_score += vt_data["malicious"] * 2
            details.append(f"VirusTotal: {vt_data['malicious']} maliciosos")

        if "error" not in abuse_data and abuse_data.get("abuse_confidence_score", 0) > 50:
    ...
    def analyze_threat_intelligence(self, ip: str) -> Dict:
        ...
        # Usar a nova função para calcular a pontuação
        threat_analysis = self._calculate_threat_score(vt_data, abuse_data)
        threat_analysis['details'] = details + threat_analysis['details'] #append de details da função _check_ip_risk
